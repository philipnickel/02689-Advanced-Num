...
fig, axes = plt.subplots(len(N_VALUES), 2, figsize=(10, 3 * len(N_VALUES)))
if len(N_VALUES) == 1:
    axes = [axes]
...
for ax, N in zip(axes, N_VALUES):
    ...
    ax[0].plot(k_vals, np.log(np.abs(coeffs_shifted)), "o-", label="FFT", linewidth=1.5)
    ax[0].plot(
        k_vals, np.log(coeffs_analytical), "--", label="Analytical", linewidth=1.5
    )

    ax[1].plot(x, u, label="Original", linewidth=1.5)
    u_reconstructed = np.real(np.fft.ifft(coeffs_fft * N))
    ax[1].plot(x, u_reconstructed, "--", label="Reconstructed", linewidth=1.5)
    style_axes(
        ax[1],
        title=f"Reconstructed vs Original (N={N})",
        xlabel="x",
        ylabel="u(x)",
        legend=True,
    )
...
ax_error.loglog(
    Ns, errors, "o-", label=r"$\max(|c_k^{\mathrm{FFT}}| - |c_k^{\mathrm{exact}}|)$"
)
style_axes(
    ax_error,
